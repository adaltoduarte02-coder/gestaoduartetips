...
exports.handler = async (event) => {
  ...
  try {
    ...
    const [homePayload, awayPayload, h2hPayload, standingsPayload] = await Promise.all([
      apiGet("/fixtures", { team: homeId, last: 10, timezone: TIMEZONE }, apiKey),
      apiGet("/fixtures", { team: awayId, last: 10, timezone: TIMEZONE }, apiKey),
      apiGet("/fixtures/headtohead", { h2h: `${homeId}-${awayId}`, last: 10, timezone: TIMEZONE }, apiKey),
      fixture.league?.id && fixture.league?.season
        ? apiGet("/standings", { league: fixture.league.id, season: fixture.league.season }, apiKey).catch(() => null)
    ...
    const standings = extractStandings(standingsPayload, homeId, awayId);
    ...
    return jsonResponse(error.status || 502, {
      ok: false,
      error: error.message || "Falha ao montar analise do jogo."
    });
  }
};
